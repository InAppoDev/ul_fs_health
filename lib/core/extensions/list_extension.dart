...
extension ListResultDataEntityExtension on List<ResultDataEntity> {
  ResultDataEntity? findObjectByBestTime() => isEmpty
      ? null
      : reduce(
          (a, b) => (a.resultTime ?? 0.0) <= (b.resultTime ?? 0.0) ? a : b);
...
extension ListExtension<T> on List<T> {
  Iterable<R> mapIndexed<R>(R Function(int index, T element) callback) =>
      indexed.map((e) => callback(e.$1, e.$2));
}
