...
class _WalkTestResultScreenState extends State<WalkTestResultScreen> {
  ...
  Widget build(BuildContext context) {
    final results = context.watch<WalkTestBloc>().state.walkResults ?? [];
    results.sort(sortWalkTestResults);
    return WalkTestResultContent(
      resultDataEntities: results,
    );
  }
}
...
class WalkTestResultContent extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return BlocListener<WalkTestBloc, WalkTestState>(
      listener: (context, state) {
        if (state.status == WalkTestStatus.failure) {
      ...
      child: TestLayoutWidget(
        ...
        children: [
          ...
          ...resultDataEntities.map((data) {
            return InfoWidget(
              color:
                  data.date?.isToday == true ? ColorScheme.of(context).primary : defaultTextColor,
